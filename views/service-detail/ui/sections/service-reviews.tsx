...
export function ServiceReviews({ reviews }: ServiceReviewsProps) {
  ...
  // `Marquee` duplicates its children to create a seamless infinite
  // scroll, which reads as visible repetition when the list is short
  // enough that both copies fit on screen at once. Threshold at 3 — at
  // that point the duplicate sits off-screen and the loop feels right.
  ...
  return (
    <section className="px-[22px] py-7">
      ...
      {useMarquee ? (
        ...
        <div className="mt-4 flex gap-3 overflow-x-auto pb-1">{cards}</div>
      )}
    </section>
  );
}
